...
namespace RFG
{
  ...
  public class AggroSpawner : MonoBehaviour
  {
    ...
    private void Update()
    {
      if (SpawnLimit != -1 && _spawnCount >= SpawnLimit)
      {
        return;
      }

      bool noCurrentInstance = _currentInstance == null || _currentInstance.gameObject == null || !_currentInstance.gameObject.activeSelf;
      bool canSpawnSeparately = Separately && noCurrentInstance;
      bool canSpawn = canSpawnSeparately || !Separately;

      if (_aggro.HasAggro && canSpawn)
      {
        if (!_canSpawn)
        {
