...
namespace RFG
{
  ...
  public class MovementAbility : MonoBehaviour, IAbility
  {
    ...
    {
      Vector2 _input = _movement.ReadValue<Vector2>();
      if (_gravityBehaviour != null)
      {
        if (_gravityBehaviour.ShouldReverseInput())
        {
          if (_gravityBehaviour.ReverseVerticalInputWhenUpsideDown)
          {
            _input.y = -_input.y;
          }
          if (_gravityBehaviour.ReverseHorizontalInputWhenUpsideDown)
          {
            _input.x = -_input.x;
          }
        }
    ...
    private void DetectSlopeMovement()
    {
      ...
      if (
        (_state.IsFacingRight && _state.BelowSlopeAngle > 0 && _horizontalInput > 0) ||
        (!_state.IsFacingRight && _state.BelowSlopeAngle < 0 && _horizontalInput < 0)
      )
      {
    ...
    private void MoveCharacter()
    {
      ...
      float movementSpeed = _horizontalInput * speed * _controller.Parameters.SpeedFactor;
